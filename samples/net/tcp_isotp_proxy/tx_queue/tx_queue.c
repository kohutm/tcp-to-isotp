...
static struct tx_queue_msg *s_alloc_tx_msg(struct tx_queue *queue)
{
	for (int i = 0; i < CONFIG_TX_MSG_QUEUE_DEPTH; ++i) {
		if (atomic_cas(&queue->used_flags[i], 0, 1)) {
...
static void s_free_tx_msg(struct tx_queue *queue, struct tx_queue_msg *msg)
{
	for (int i = 0; i < CONFIG_TX_MSG_QUEUE_DEPTH; ++i) {
		if (&queue->msg_pool[i] == msg) {
			atomic_set(&queue->used_flags[i], 0);
 ...
 * It waits for new data to be available and then processes each message
 * by calling the user data handler and the send callback if they are set.
...
static void tx_msg_queue_process(void *arg1, void *arg2, void *arg3)
{
	...
	while (1) {
		...
		SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&tx_queue_sys.global_queue_list,
						  queue, tmp, node) {
			while (true) {
...
tx_msg_queue_err_t tx_queue_unregister(struct tx_queue *queue)
{
	...
	atomic_dec(&tx_queue_sys.queue_count);
