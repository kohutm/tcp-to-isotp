/*
 * TCP Command Handler
 * This header file defines the TCP command handler interface for handling commands over TCP connections.
 * It includes error codes, command types, and function prototypes for initializing, registering,
 * unregistering, and executing command handlers.
 ...
 * It also includes error handling to ensure that commands are processed correctly and that errors are reported.
 * The command handler is designed to be thread-safe and can be used in a multi-threaded environment.
 */
#pragma once
 ...
 */
#define TCP_CMD_HANDLER_BUFFER_SIZE  (CONFIG_TCP_CMD_HANDLER_DATA_MAX_LEN + CONFIG_TCP_CMD_HANDLER_NAME_MAX_LEN + \
                                     CONFIG_TCP_CMD_HANDLER_VAL_MAX_LEN + CONFIG_TCP_CMD_HANDLER_SPACES_MAX_LEN + \
