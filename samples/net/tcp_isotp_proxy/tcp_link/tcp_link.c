...
static tcp_cmd_err_t s_tcp_link_cmd_handler_echo_name_value(const char *cmd_name, int value) {
	tx_msg_queue_err_t err = TX_MSG_QUEUE_ERR_OK;
    char buf[CONFIG_TCP_CMD_HANDLER_NAME_MAX_LEN + CONFIG_TCP_CMD_HANDLER_VAL_MAX_LEN + 2]; // +2 for space and null terminator
    snprintf(buf, sizeof(buf), "%s %d", cmd_name, value);
	err = tx_queue_msg_send(&tcp_link.tcp_tx_queue, buf, strlen(buf));
	if (err != TX_MSG_QUEUE_ERR_OK) {
		LOG_ERR("Failed to send command name '%s' with value '%d' to TX queue: %s", cmd_name, value,
			tcp_link.tcp_tx_queue.name);
		return TCP_CMD_ERR_HANDLER;
	}
	return TCP_CMD_ERR_OK;
}
...
static tcp_cmd_err_t s_tcp_link_cmd_handler_data_echo(const char *cmd_name, const char *data, size_t len) {
	tx_msg_queue_err_t err = TX_MSG_QUEUE_ERR_OK;

    err = tx_queue_msg_send(&tcp_link.tcp_tx_queue, data, len);
    if (err != TX_MSG_QUEUE_ERR_OK) {   
        LOG_ERR("Failed to send data '%.*s' to TX queue: %s", (int)len, data, tcp_link.tcp_tx_queue.name);
        return TCP_CMD_ERR_HANDLER;
    }

	return TCP_CMD_ERR_OK;
}
static tcp_cmd_err_t s_tcp_link_cmd_handler_data_print(const char *cmd_name, const char *data, size_t len) {
	char buf[(CONFIG_TCP_CMD_HANDLER_DATA_MAX_LEN * 2) + 1]; // 2 chars per byte, plus null terminator
	size_t i, pos = 0;

	LOG_INF("Command received: %s with data length: %d", cmd_name, (int)len);

	if (len > CONFIG_TCP_CMD_HANDLER_DATA_MAX_LEN) len = CONFIG_TCP_CMD_HANDLER_DATA_MAX_LEN; // prevent buffer overflow
...
static tcp_cmd_err_t s_tcp_link_cmd_handler_data_to_isotp(const char *cmd_name, const char *data, size_t len) {
    struct tx_queue *isotp_tx_queue = tx_queue_get_by_name(CONFIG_CAN_ISOTP_PROXY_TX_QUEUE_NAME);
    ...
    if (tx_queue_err != TX_MSG_QUEUE_ERR_OK) {
        LOG_ERR("Failed to send data '%.*s' to ISO-TP TX queue: %s", (int)len, data, isotp_tx_queue->name);
        return TCP_CMD_ERR_HANDLER;
    }
...
static void s_tcp_link_register_tcp_cmd_handlers(void) {
	...
	err = tcp_cmd_handler_register_cb("name-echo", TCP_CMD_NAME_ONLY, s_tcp_link_cmd_handler_echo_cmd_name);
	if (err != TCP_CMD_ERR_OK) {
		LOG_ERR("Failed to register name-echo handler: %d", err);
	}
	err = tcp_cmd_handler_register_cb("name-print", TCP_CMD_NAME_ONLY, s_tcp_link_cmd_handler_print_cmd_name);
	if (err != TCP_CMD_ERR_OK) {
	...
	err = tcp_cmd_handler_register_cb("data-echo", TCP_CMD_DATA, s_tcp_link_cmd_handler_data_echo);
	if (err != TCP_CMD_ERR_OK) {
		LOG_ERR("Failed to register data-echo handler: %d", err);
	}
	err = tcp_cmd_handler_register_cb("data-print", TCP_CMD_DATA, s_tcp_link_cmd_handler_data_print);
	if (err != TCP_CMD_ERR_OK) {
		LOG_ERR("Failed to register data-print handler: %d", err);
	}
	err = tcp_cmd_handler_register_cb("data-to-isotp", TCP_CMD_DATA, s_tcp_link_cmd_handler_data_to_isotp);
	if (err != TCP_CMD_ERR_OK) {
...
#else
            int len = recv(tcp_link.client_fd, tcp_link.recv_buf, sizeof(tcp_link.recv_buf), 0);
#endif
            if (len <= 0) {
...
#else
    if (k_mutex_lock(&tcp_link.tx_lock, K_FOREVER) != 0) {
